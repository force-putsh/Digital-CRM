...
namespace Crm.Pages
{
    public partial class EditContratComponent : ComponentBase
    {
        ...
        {
            var crmdataGetContratByIdResult = await Crmdata.GetContratById(id:Id);
            contrat = crmdataGetContratByIdResult;
        ...
        protected async System.Threading.Tasks.Task Form0Submit(Crm.Models.Crmdata.Contrat args)
        {
            try
            {
                var crmdataUpdateContratResult = await Crmdata.UpdateContrat(id:Id, contrat:contrat);
                DialogService.Close(contrat);
            }
            catch (System.Exception crmdataUpdateContratException)
            {
                NotificationService.Notify(new NotificationMessage(){ Severity = NotificationSeverity.Error,Summary = $"Error",Detail = $"Unable to update Contrat" });
            }
        }
