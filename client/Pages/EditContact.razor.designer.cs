...
namespace Crm.Pages
{
    public partial class EditContactComponent : ComponentBase
    {
        ...
        {
            var crmdataGetContactByIdResult = await Crmdata.GetContactById(id:Id);
            contact = crmdataGetContactByIdResult;
        }
        ...
        protected async System.Threading.Tasks.Task Form0Submit(Crm.Models.Crmdata.Contact args)
        {
            try
            {
                var crmdataUpdateContactResult = await Crmdata.UpdateContact(id:Id, contact:contact);
                DialogService.Close(contact);
            }
            catch (System.Exception crmdataUpdateContactException)
            {
