...
namespace Crm.Pages
{
    public partial class AddStatutTacheComponent : ComponentBase
    {
        ...
        {
            statuttache = new Crm.Models.Crmdata.StatutTache(){};
        }
        ...
        protected async System.Threading.Tasks.Task Form0Submit(Crm.Models.Crmdata.StatutTache args)
        {
            try
            {
                var crmdataCreateStatutTacheResult = await Crmdata.CreateStatutTache(statutTache:statuttache);
                DialogService.Close(statuttache);
            }
            catch (System.Exception crmdataCreateStatutTacheException)
            {
                NotificationService.Notify(new NotificationMessage(){ Severity = NotificationSeverity.Error,Summary = $"Error",Detail = $"Unable to create new StatutTache!" });
            }
        }
