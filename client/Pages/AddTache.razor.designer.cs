...
namespace Crm.Pages
{
    public partial class AddTacheComponent : ComponentBase
    {
        ...
        protected async System.Threading.Tasks.Task Load()
        {
            ...
            tache = new Crm.Models.Crmdata.Tache(){};
        }
        ...
        protected async System.Threading.Tasks.Task Form0Submit(Crm.Models.Crmdata.Tache args)
        {
            try
            {
                var crmdataCreateTacheResult = await Crmdata.CreateTache(tache:tache);
                DialogService.Close(tache);
            }
            catch (System.Exception crmdataCreateTacheException)
            {
                NotificationService.Notify(new NotificationMessage(){ Severity = NotificationSeverity.Error,Summary = $"Error",Detail = $"Unable to create new Tache!" });
            }
        }
